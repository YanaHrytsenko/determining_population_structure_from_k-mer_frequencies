...
targets = ['AMR_PEL', 'EAS_JPT', 'EUR_TSI', 'SAS_ITU'] #make sure they are listed in the correct order
colors = ['r', 'g', 'b', 'y'] #['b', 'g', 'r', 'c', 'm', 'y', 'k', 'w']
...
plt.savefig("2D_PCA_4_populations_24_samples_from_kmer_freqs.pdf")
...
for k in range(1,10):
    ...
    model.fit(finalDf.iloc[:,:2])#did with 2 before but original was 3 and I am not sure why 3, because range is not inclusinve?
...
plt.ylabel('inertia')
plt.xticks(ks)
plt.savefig("elbow_method_plot_k_means_4_superpops_21_mer_param.pdf")
plt.clf() #clear the plt variable so the plots don't overlap

model = KMeans(n_clusters=4)#4 because the elbow method shows the breaking point at 4
...
plt.savefig("k_means_clustering_4_superpops_21_mers_param.pdf")
